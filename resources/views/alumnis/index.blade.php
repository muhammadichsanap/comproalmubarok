...
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>Alumni Sekolah</h1>
                </div>
                <div class="col-sm-6">
                    <div class="float-right">
                        <form action="{{ route('alumnis.deleteAll') }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger" onclick="return confirm('Apakah anda yakin ingin menghapus SEMUA DATA?')">
                                Hapus Semua Data
                            </button>
                        </form>
                        <a class="btn btn-primary" href="{{ route('alumnis.create') }}">
                            Tambah
                        </a>
                        @if($alumnis->isNotEmpty())
                            <a href="{{ route('alumnis.export') }}" class="btn btn-info">
                                Export CSV
                            </a>
                        @else
    ...
    <div class="content px-3">
        ...
        <div class="card">
            <div class="card-body p-0">
                @include('alumnis.table')

                <div class="card-footer clearfix">
                    <div class="float-right">
                        @include('adminlte-templates::common.paginate', ['records' => $alumnis])
                    </div>
                </div>
            </div>
        </div>
